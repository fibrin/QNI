...
function segBW = chenvese(I,mask,num_iter,mu,method)
  ...
  persistent  chanfigureH
  
  if isempty(chanfigureH)
     chanfigureH=figure();
  end
  ...
  segBW=zeros(size(I));
  [ssy,ssx] = size(seg);
  if ssy>sy,ssy=sy;end;
  if ssx>sx,ssx=sx;end;
  segBW(1:ssy,1:ssx)=seg(1:ssy,1:ssx);
