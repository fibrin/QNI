...
function [seg,stack,WaterMass] = chenvese_it(I0,mask,num_iter,mu,method,option)
...
try Iref=option.Iref; end
try TH=option.TH; end
try ImageNum=option.ImageNumber; end
segf=[];
...
seg=[];
stack{num_iter}=[];
WaterMass=zeros(num_iter,1);

%%
%-- Initializations on input image I and mask
%  resize original image
   s = 200./min(size(I0,1),size(I0,2)); % resize scale
   if s<1
       I = imresize(I0,s);
   else
      I=I0;
    ...
    case {'chan','vector'}
        ...
        %-- Main loop
          for n=1:num_iter
              ...
              % get stepsize dt what is it 
              dt=0.5; %original
              %dt=10; %cs
              

              % intermediate output
              if(mod(n,20) == 0) && verbose 
                 showphi(I,phi0,n);  
              end;
              ...
              if ~isempty(Iref)
                 if s<1
                   seg=mapseg(seg,size(I0));
                 end
                 stack{n}=seg;
                 segf=ImageShow(seg,strcat('Seg ',num2str(ImageNum)),segf,0,[],[],1);
                 WaterMass(n)=sum(sum(Iref.*seg));
              end
              ...
              if indicator && n>5 % decide to stop or continue 
                  break;  %=>>>> exit for 
              end
          end;
          ...
          if verbose
            segf=figure;
            plot(WaterMass);
            SaveVars(strcat('WaterMass',num2str(ImageNum)),WaterMass);
            showphi(I,phi0,n);
...
function seg=mapseg(segin,sysx)
    [sy,sx]=size(segin);
    sy1=sysx(1);sx1=sysx(2);
    scale=(sy1/sy+sx1/sx)/2;
    segx = imresize(segin,scale);
    [sy2,sx2]=size(segx);
    seg=logical(zeros(sysx));
    if sx2>sx1,sx2=sx1;end;
    if sy2>sy1,sy2=sy1;end;
    seg(1:sy2,1:sx2)=segx(1:sy2,1:sx2);
end
